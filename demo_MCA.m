...
imgRoot1 = './saliencymap1DBD/ASD/';
imgRoot2 = './saliencymap2YS_NFGF/ASD/';
imgRoot3 = './saliencymap3WL/ASD/';
imgRoot4 = './saliencymap4GR/ASD/';
imgRoot5 = './saliencymap5MR/ASD/'; % the input saliency maps paths
saldir = './saliencymapMCA_NFGF/ASDFGF_MCA/';% the output path of the saliency map
mkdir(saldir);
...
for ii = 1:length(imnames1)
    ...
    outname=[saldir imnames1(ii).name(1:end-4) '_Ours' '.png'];
    imwrite(S_N2,outname); 
    %% 执行快速指导滤波器FGF
    I = double(imread([saldir imnames1(ii).name(1:end-4) '_Ours' '.png']))/255 ;%灰度图
    p = I;
    r = 4; % try r=2, 4, or 8
    ...
    % imshow(salmaps);
    outname=[saldir imnames1(ii).name(1:end-4) '_OursFGF' '.png'];
