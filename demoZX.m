...
addpath(genpath('.'));
%% paramter
deltaw=0.05; %% controal edge weight控制边缘权重
spnumber=200;%% superpixels number超像素数目
deltap=0.2;
deltax=0.15;
deltay=0.15;
lamda=25;
...
nPoints=30;
%
imgRoot='./test/ASD/';%% test image dir
outdir='./saliencymaps/ASD/';%% the output saliency map saving dir
...
%% generate superpixels
    ...
    spnum=max(max(superpixels));  %真正的超像素数目 
...
%% assign the saliency value to each pixel
    ...
    outname=[outdir imnames(ii).name(1:end-4) '_GR' '.png'];   
